                    ...
                    <div>
                        {
                            reviews?.map(review => <SwiperSlide key={review?._id}>
                                ...
                                <div className='flex flex-col max-w-3xl mx-auto justify-center items-center gap-2'>
                                    ...
                                    /> */}
                                    <p className=' mt-5 text-2xl font-bold'> {review?.reviewProperty} </p>
                                    <h2> Agent Name: <span className="font-semibold"> {review?.agentName} </span> </h2>
                                    <p className='font-semibold '>{review?.date}</p>
                                    <button onClick={() => handleDelte(review?._id)} className="btn btn-error text-white">Delete</button>
                                </div>
                            </SwiperSlide>)
                        }
                    </div>
