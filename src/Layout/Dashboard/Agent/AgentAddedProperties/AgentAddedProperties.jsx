      ...
      <div>
        ...
        <div className="grid grid-cols-1 pr-3 md:grid-cols-2 max-w-7xl gap-5 my-12 mx-auto lg:grid-cols-4">
          {properties?.map((item) => (
            <Card
              key={item?._id}
              sx={{ maxWidth: "380px", mx: "auto", position: "relative" }}
            >
              <CardActionArea>
                <CardMedia
                  component="img"
                  height="140"
                  image={
                    item?.images?.[0] || "https://i.ibb.co/Xf5XstKT/images.jpg"
                  }
                  alt="house picture"
                />
                <CardContent>
                  ...
                  <p className="text-[#F2561B] font-semibold my-1 text-[16px]">
                    {" "}
                    ...
                    {item?.status ? (
                      <div>
                        {item?.status === "verified" ? (
                          <button className="bg-[#F2561B] btn btn-sm text-white">
                            {" "}
                            {item?.status}{" "}
                          </button>
                        ) : (
                      ...
                      <span className="btn btn-sm btn-primary">pending </span>
                    )}
                  </div>
                </CardContent>
              </CardActionArea>
              <div className="flex justify-between  items-center py-1 gap-3 w-full ">
                ...
                {item.status === "rejected" ? (
                  <button className="py-1 px-4 mr-1 duration-300 opacity-30 hover:bg-white hover:text-[#F2561B] flex gap-1 justify-end border  text-[#F2561B] border-[#F2561B]">
                    Update
                  </button>
                ) : (
                  <button
                    onClick={() => handleUpdate(item)}
                    className="py-1 px-4 mr-1 duration-300 disabled:opacity-30 disabled:hover:bg-white disabled:hover:text-[#F2561B] flex gap-1 hover:bg-[#F2561B] hover:text-white justify-end border  text-[#F2561B] border-[#F2561B]"
                  >
